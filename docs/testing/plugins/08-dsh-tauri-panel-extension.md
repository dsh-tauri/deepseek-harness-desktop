...
> 自动化：`packages/dsh-tauri-panel-extension/test/extension-routes.e2e.ts`（待建立）；客户端与 L3 见各用例标注
...
[自动化] 是（`packages/dsh-tauri-panel-extension/test/extension-routes.e2e.ts`）
